...
Публикацию блокирует только **CRITICAL, для которого есть исправление**.
`MEDIUM` и `HIGH` попадают во вкладку Security и разбираются там, не мешая
релизу. Порог выбран так намеренно: половина образов здесь стоит на чужих
базах — `jenkins/jenkins`, `linuxserver/plex`, `postgres` — и `HIGH` в них
есть почти всегда независимо от нашего кода. Сборка, падающая каждый раз, не
защищает, а приучает отключать проверку.

Исключения — в [`.trivyignore`](.trivyignore), один файл на репозиторий.
