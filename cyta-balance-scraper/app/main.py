...
#
# Раньше basicConfig не вызывался вовсе. Корневой логгер оставался с
# уровнем WARNING, поэтому из скрепера в вывод попадала ровно одна строка —
# «refresh failed: ...», — а весь ход входа (какая страница открылась, чем
# заполнились поля, куда вернуло после submit) оставался невидимым.
#
# 09.09.2026 это стоило лишнего круга работы: причину перемежающегося отказа
# входа пришлось искать, запуская тот же код руками, потому что в логе пода
# следа не было. Уровень берётся из LOG_LEVEL, по умолчанию INFO.
logging.basicConfig(
